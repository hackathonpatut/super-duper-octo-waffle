...
export default withRouter(
  ...
  render() {
      ...
      const totalPrice = items
        .reduce((p, v) => p + v.amount * v.price, 0)
        .toFixed(2);
      ...
      const totalHealth =
        totalInCart !== 0
          ? items.reduce((p, v) => p + v.amount * v.health, 0) / totalInCart
          : 0;
      const totalSustainability =
        totalInCart !== 0
          ? items.reduce((p, v) => p + v.amount * v.sustainability, 0) /
            totalInCart
          : 0;
          ...
          <Card>
            <Card.Content>
              ...
              <div className="progress">
                <Progress
                  value={Math.round(totalSustainability * 100)}
                  hideText={true}
                  type="sustainable"
                />
                Average
              ...
              <div className="progress">
                <Progress
                  value={Math.round(totalHealth * 100)}
                  hideText={true}
                  type="health"
                />
                Average
